...
only need `test.mly`, to help test such behavior in development mode, users could set it manually
...
Then `bsb` will not re-generate `test.ml` whenever `test.mli` changes.